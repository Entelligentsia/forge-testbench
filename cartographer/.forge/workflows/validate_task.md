...
3. Validation:
   ...
   - When re-running the test suite, use the **resolved test command** from `commands.test` in `.forge/config.json` (i.e. `` `${commands.test}` ``, e.g. `.venv/bin/python -m pytest`). Template placeholder: npm test. Do NOT invoke bare `python` / `python3` — the project interpreter is rarely on `$PATH`.
