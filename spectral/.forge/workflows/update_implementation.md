...
3. Implementation:
   ...
   - Verify the changes using the **resolved test command** from `commands.test` in `.forge/config.json` (i.e. `` `${commands.test}` ``, e.g. `.venv/bin/python -m pytest`). Template placeholder: python3 -m pytest. Do NOT invoke bare `python` / `python3` — the project interpreter is rarely on `$PATH`.
