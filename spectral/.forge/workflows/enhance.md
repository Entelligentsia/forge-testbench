...
(e.g., `scripts.test` in `package.json` is the sole candidate for `python3 -m pytest`). It is
...
3. **Derive values from codebase signals** — for each remaining `{{KEY}}`, attempt to derive a
   ...
   | Key | Signal source |
   |-----|--------------|
   | `` | `package.json` dependencies field (list top-level frameworks); or dominant file extension survey |
   | `` | `git branch -a` output pattern or `.git/config` |
   | `python3 -m pytest` | `package.json` → `scripts.test` |
   | `{{BUILD_COMMAND}}` | `package.json` → `scripts.build` |
   | `` | Scan source for ORM model files or type definitions |
   | `` | Top-level directory listing (exclude `.git`, `node_modules`, `.forge`) |
   | `` | Derive from project type (web app → UI/API/DB/Infra; library → API/Docs/Tests) |
...
3. **Compare**: for each structural element, verify:
   ...
   - It uses valid `engineering/architecture` references (paths that exist in `engineering/`).
